...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{work_flow.png}
\caption{Overall Workflow of Proposed Mobility Prediction System}
\label{fig:workflow}
\end{figure}
...
\caption{Flowchart of task offloading covering all 6 cases.}
\label{fig:offloading_flowchart}
\end{figure}

The proposed MA-DDPG agent aims to proactively optimize these caching placements and continuous offloading ratios ($o_v, o_e$) to minimize the total system delay, explicitly avoiding high-latency scenarios such as Case 5 and Case 6.
...
for i, line in enumerate(lines):
    if line.strip() == r'\chapter{Methodology}':
        start_idx = i - 1 # Include the %------ line above it
        break

for i in range(start_idx+1, len(lines)):
    if line.strip() == r'\chapter{Results}':
        end_idx = i - 1
        break
...
if start_idx != -1 and end_idx != -1:
    ...
    print(f"Successfully replaced methodology content")
else:
